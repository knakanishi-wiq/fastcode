# Requirements Archive: v1.3 SQLite FTS5 BM25 Migration
...
- [x] **EMB-01**: An `embedding_cache` table in SQLite stores embeddings keyed on `(content_hash, model)` with the embedding stored as a BLOB
- [x] **EMB-02**: `CodeEmbedder` checks the SQLite `embedding_cache` before calling `litellm.embedding()`; cache hits avoid a VertexAI API round-trip
...
| EMB-01 | Phase 14 | Complete |
| EMB-02 | Phase 14 | Complete |
